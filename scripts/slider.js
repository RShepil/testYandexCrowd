class Carousel {
  ...
  updateSelectBox = () => {
    const selectButtons = this.carousel.querySelectorAll(
      ".controls__select-btn",
    );
    let visibleSlideID = this.counter;
    let currentSelectBtn = [...selectButtons].find(
      (btn) => btn.dataset.id === visibleSlideID.toString(),
    );
