class InfiniteCarousel {
  ...
  calcVisibleSlidesNumber() {
    const carouselInnerWidth = parseFloat(
      window.getComputedStyle(this.carousel).width,
    );
    const visibleSlides = Math.round(
      carouselInnerWidth / this.slideInitialWidth,
    );
  ...
  updateCounter = () => {
    const carouselContainer = this.carousel.querySelector(
      ".inf-carousel__container",
    );
    let [curr, total] =
      this.carousel.querySelector(".controls__counter").children;
  ...
  moveSlide = () => {
    ...
    const containerGap = parseFloat(
      window.getComputedStyle(carouselContainer).gap,
    );
